...
### (warning)️ IN PROGRESS: Block Lanczos Method
...
**Current Status**: (warning)️ **NUMERICAL ISSUES** - eigenvalues diverge (error ~5e10)
...
| LanczosEigensolver  | (ok)    | (warning)️  | (ok)    | WORKING (JAX float32) |
| ArnoldiEigensolver  | (ok)    | (ok)  | (ok)    | WORKING |
| BlockLanczos        | (warning)️    | (warning)️  | (ok)    | BROKEN (SciPy works) |
...
(warning)️ Block Lanczos:           SKIPPED (numerical issues)
...
- (warning)️ Block Lanczos (SciPy only - native implementation needs fixing)
