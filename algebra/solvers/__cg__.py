    ...
    def solve(self, b, x0: Optional[np.ndarray] = None, precond: Optional[Preconditioner] = None):
        ...
        try:
            if precond is not None:
                self.set_preconditioner_sigma(self.reg)
                if _take_jax:
                    self._solution, self._converged = self.solve_precond_jit(self._mat_vec_mult, b, x0, precond)
                ...
                if _take_jax:
                    self._solution, self._converged = self.solve_no_precond_jit(self._mat_vec_mult, b, x0)
                    return self._solution
                else:
