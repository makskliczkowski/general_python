...
# Also add dtype instances (e.g., np.dtype('float64'))
_TYPE_TO_NAME[np.dtype('complex64')]    = 'complex64'
_TYPE_TO_NAME[np.dtype('complex128')]   = 'complex128'
_TYPE_TO_NAME[np.dtype('float32')]      = 'float32'
_TYPE_TO_NAME[np.dtype('float64')]      = 'float64'
_TYPE_TO_NAME[np.dtype('int32')]        = 'int32'
_TYPE_TO_NAME[np.dtype('int64')]        = 'int64'
...
def _qes_initialize_utils():
    ...
    # Configure spin value from environment
    env_spin_value                  = os.getenv(PY_SPIN_VALUE_STR, "").strip()
    if env_spin_value:
        try:
            BACKEND_REPR            = float(env_spin_value)
            _log_message(f"Spin value set from environment PY_SPIN_VALUE={BACKEND_REPR}", 1)
        except ValueError:
            _log_message(f"Invalid PY_SPIN_VALUE '{env_spin_value}', using default {BACKEND_REPR}", 0)
    else:
