...
class _FlaxRBM(nn.Module):
    ...
    def __call__(self, s: jax.Array) -> jax.Array:
        ...
        where v is the visible layer state (input `s`), W are weights,
        b is hidden bias, a is visible bias (often omitted or fixed).
        ...
        visible_state = s.astype(self.dtype) # Ensure correct dtype
        ...
        if jnp.issubdtype(self.param_dtype, jnp.complexfloating):
            # Get the complex initializer function (using Lecun normal scaling for example)
            kernel_init_fn  = cplx_variance_scaling(1.0, 'fan_in', 'normal', dtype=self.param_dtype)
            bias_init_fn    = nn.initializers.zeros 
        else:
            # Get the real initializer function
            kernel_init_fn  = lecun_normal(dtype=self.param_dtype)
            bias_init_fn    = nn.initializers.zeros

        # Define the Dense layer (Visible to Hidden weights W and hidden bias b)
        ...
        # log_cosh handles complex inputs correctly: log(cosh(x+iy))
        ...
        # Note: Visible bias term 'a*v' is omitted here.
...
class RBM(FlaxInterface):
    ...
    def __init__(self,
                ...
                n_hidden       : int     = 2,
                bias           : bool    = True,
                map_to_pm1     : bool    = True,               # Flag to map {0,1} -> {-1,1}
        ...
        if is_complex:
            if not jnp.issubdtype(final_dtype, jnp.complexfloating) or not jnp.issubdtype(final_param_dtype, jnp.complexfloating):
                raise ValueError("Complex RBM requires complex dtype and param_dtype.")
        else:
            if jnp.issubdtype(final_dtype, jnp.complexfloating) or jnp.issubdtype(final_param_dtype, jnp.complexfloating):
                raise ValueError("Real RBM requires real dtype and param_dtype.")
    ...
    def __repr__(self) -> str:
        ...
        return (f"{rbm_type}RBM(input={self.input_shape}, hidden={self._net_kwargs_in.get('n_hidden', '?')}, "
                f"bias={self._net_kwargs_in.get('bias', '?')}, dtype={self.dtype}, "
                f"params={self.num_parameters}, status={init_status})")
