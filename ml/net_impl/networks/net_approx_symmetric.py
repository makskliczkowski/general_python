...
This module implements the "nonsymmetric χ -> symmetric Ω -> invariant σ" architecture pattern.
...
class ApproxSymmetricNet(nn.Module):
    ...
    Attributes:
        chi_block: The nonsymmetric block (e.g., a Dense layer or MLP).
        symmetry_op: A callable that applies the symmetry operation (e.g., group averaging).
                     Signature: (x: Array) -> Array
        readout_act: The invariant nonlinearity (sigma).
        dtype: The dtype for computation.
    """
    chi_features: Sequence[int]
    symmetry_op: Callable[[jnp.ndarray], jnp.ndarray]
    readout_act: Callable[[jnp.ndarray], jnp.ndarray] = jnp.log
    chi_act: Callable[[jnp.ndarray], jnp.ndarray] = nn.relu
